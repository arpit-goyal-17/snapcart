...
import { ArrowLeft, Building, CreditCard, CreditCardIcon, Home, Loader2, LocateFixed, MapPin, Navigation, Phone, Search, TruckIcon, User } from 'lucide-react'
...
function CheckOut() {
    ...
    const disableButton=!address.fullName||!address.mobile||!address.city||!address.city||!address.state||!address.pincode||!address.fullAddress||address.pincode.length<6
    ...
    useEffect(() => {
        if (userData) {
            setAdress((prev) => ({ ...prev, fullName: userData?.name || "" }))
            setAdress((prev) => ({ ...prev, mobile: userData?.mobile || "" }))
        }
    }, [userData])
    ...
    useEffect(()=>{
        const fetchAddress=async ()=>{
            try{
                ...
                setAdress(prev=>({...prev,city:result.data.address.city,state:result.data.address.state,pincode:result.data.address.postcode,fullAddress:result.data.display_name}))
            }
            catch(error){
                console.log(error)
            }
    ...
    return (
      ...
      font-semibold' onClick={() => router.push("/user/cart")}>
                <ArrowLeft size={16} />
            ...
            duration-300 p-6 border border-gray-100'
                >
                    ...
                    <div className='space-y-4'>
                        <div className='relative'>
                            ...
                            <input type="text" value={address.fullName} onChange={(e) => setAdress((prev) => ({
                                ...prev,
                        ...
                        <div className='relative'>
                            ...
                            <input type="text" value={address.mobile} onChange={(e) => setAdress((prev) => ({
                                ...prev,
                        ...
                        <div className='relative'>
                            ...
                            <input type="text" value={address.fullAddress} placeholder='Full Adress'
                        ...
                        focus:ring-green-500 outline-none' onChange={(e)=>{
                    ...
                    border-gray-200 shadow-inner'>
                            {position&&<MapContainer center={position as LatLngExpression} zoom={13} scrollWheelZoom={true}
                ...
                duration-300 p-6 border border-gray-100 h-fit'>
                    ...
                    <motion.button
                    whileTap={{scale:0.93}}
                    className={`w-full mt-6 text-white py-3 rounded-full transition-all font-semibold ${
                        disableButton?
                        "bg-gray-400":
                    ...
                    onClick={()=>{
                        ...
                        {disableButton?"Fill Address Details":paymentMethod=="cod"?"Place Order":"Pay and place order"}
                    </motion.button>
                </motion.div>
            </div>
        </div>
