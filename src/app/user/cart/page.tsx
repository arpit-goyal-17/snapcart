...
function CartPage() {
  ...
  return (
    <div className='w-[95%] sm:w-[90%] md:w-[80%] mx-auto mt-8 mb-24 relative'>
        <Link href={"/"} className='absolute -top-2 left-0 flex item-center gap-2 text-green-700
        ...
        className='text-center py-20 bg-white rounded-2xl shadow-md'>
            ...
            <p className='text-gray-600 text-lg mb-6'>Your cart is empty. Add some groceries 
                to continue shopping</p>
            <Link href={"/"} className='bg-green-600 text-white px-6 py-3 rounded-full 
            hover:bg-green-700 transition-all inline-block font-medium'>Continue shopping</Link>
        </motion.div>
                ...
                {cartData.map((item,index)=>(
                    ...
                    className='flex flex-col sm:flex-row items-center bg-white rounded-2xl
                     shadow-md p-5 hover:shadow-xl transition-all duration-300 border border-gray-100'>
                        <div className='relative w-28 h-28 sm:h-24 sm:w-24 md:h-28 md:w-28 flex-shrink-0
                        rounded-xl overflow-hidden bg-gray-50'>
                            <Image src={item.image} alt={item.name} fill
                            className='object-contain p-3 trantion-transform duration-300 hover:scale-105'
                            />
                        </div>
                        <div className='mt-4 sm:mt-0 sm:ml-4 flex-1 text-center sm:text-left'>
                            <h3 className='text-base sm:text-lg font-semibold text-gray-800
                            line-clamp-1'>{item.name}</h3>
                            <p className='text-xs sm:text-sm text-gray-500'>{item.unit}</p>
                            <p className='text-green-700 font-bold mt-1 text-sm sm:text-base'>
                                ₹{item.quantity*Number(item.price)}</p>
                        </div>
                        <div className='flex items-center justify-center sm:justify-end gap-3 
                        mt-3 sm:mt-0 bg-gray-50 px-3 py-2 rounded-full'>
                            <button className='bg-white p-1.5 rounded-full hover:bg-green-100
                            transition-all border border-gray-200'
                            onClick={()=>dispatch(decreaseQuantity(item._id))}><Minus size={14} 
                            className='text-green-700'/></button>
            ...
            className='bg-white rounded-2xl shadow-xl p-6 h-fit sticky top-24 border 
