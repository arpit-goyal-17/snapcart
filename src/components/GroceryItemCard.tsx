...
function GroceryItemCard({item}:{item:IGrocery}) {
  ...
  return (
    ...
    overflow-hidden border-gray-100 flex flex-col'
    >
        ...
        <div className='flex items-center justify-between mt-2'>
            ...
            >{item.unit}</span>
            <span
            className='text-green-700 font-bold text-lg'> ₹{item.price}</span>
        </div>
        {!cartItem?<motion.button
        whileHover={{scale:1.05}}
        whileTap={{scale:0.96}}
        transition={{}}
        className='mt-4 flex items-center justify-center gap-2 bg-green-600 
        hover:bg-green-700 text-white rounded-full py-2 text-sm font-medium trantion-all'
        ...
        className='mt-4 flex items-center justify-center bg-green-50 border border-green-200
       rounded-full py-2 px-4 gap-4'>
        ...
        <button className='w-7 h-7 flex items-center justify-center rounded-full bg-green-100
        hover:bg-green-200 transition-all'
        onClick={()=>dispatch(increaseQuantity(item._id))}><Plus size={16} className='text-green-700'/></button>
        </motion.div>
        }
        </div>
    </motion.div>
