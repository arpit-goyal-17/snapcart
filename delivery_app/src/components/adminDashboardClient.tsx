  ...
  return (
    <div className='pt-28 w-[90%] md:w-[80%] mx-auto'>
      <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-10 text-center sm:text-left'>
        ...
        className='text-3xl md:text-4xl font-bold text-green-700'>🛒 Admin Dashboard</motion.h1>
        <select className='border border-gray-300 px-4 py-2 text-sm focus:ring-2 focus:ring-green-500 outline-none transition w-full sm:w-auto' onChange={(e)=>setFilter(e.target.value as any)} value={filter}>
      ...
      <div className='grid gird-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10'>
        {stats.map((s,i)=>{
            const icons=[
