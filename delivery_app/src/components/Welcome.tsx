...
function Welcome({nextStep}:propType) {
  return (
    <div className='flex flex-col items-center justify-center min-h-screen text-center p-6'>
      ...
      <motion.button initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.6,delay:0.8}}
      className='inline-flex items-center justify-center gap-2 bg-green-600 hover:bg-green-800 text-white font-semibold 
       py-3 px-8 rounded-2xl shadow-md trantion-all duration-200 mt-10'
       onClick={()=>nextStep(2)}>
        Next
        <ArrowRight />
      </motion.button>
