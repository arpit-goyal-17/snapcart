...
function Nav({ user }: { user: IUser }) {
  ...
  const sidebar=menuOpen?createPortal(
    <AnimatePresence>
      ...
      backdrop-blur-xl border-r border-green-400/20 shadow-[0_0_50px_-10px_rgba(0,255,100,0.3)]
      flex flex-col p-6 text-white'>
        ...
        transition-all shadow-inner'>
          ...
          shadow-lg'
          >{user.image ? <Image src={user.image} alt="user" fill className="object-cover rounded-full" />
          : <User />}</div>
          <div>
          ...
          <div className='flex flex-col gap-3 font-medium mt-6'>
            <Link href={"/admin/addGrocery"} className='gap-3 p-3 flex items-center rounded-lg bg-white/10 
            hover:bg-white/20 hover:pl-4 transition-all'>
            <PlusCircle className='w-5 h-5'/>
          Add Grocery
          </Link>
          <Link href={"/admin/view-grocery"} className='gap-3 p-3 flex items-center rounded-lg bg-white/10 
            hover:bg-white/20 hover:pl-4 transition-all'>
  ...
  return (
    ...
    rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50'>
      ...
      {user.role == "user" &&
        <form className='hidden md:flex items-center bg-white rounded-full px-4 py-2
      w-1/2 max-w-lg shawdow-md' onSubmit={handleSearch}>
          <Search className='text-gray-500 w-5 h-5 mr-2' />
          <input type="text" placeholder="Search for groceries"
            className='w-full outline-none text-gray-700 placeholder-gray-400' onChange={(e)=>{
      ...
      <div className='flex items-center gap-3 md:gap-6 relative'>
        ...
        {user.role=="user" && <><div className='bg-white rounded-full w-11 h-11 flex items-center
        justify-center shadow-md hover:scale-105 transition md:hidden'
        ...
        <div className='hidden md:flex items-center gap-4'>
          <Link href={"/admin/addGrocery"} className='flex items-center gap-2 bg-white text-green-700 font-semibold
          px-4 py-2 rounded-full hover:bg-green-100 transition-all'>
            <PlusCircle className='w-5 h-5'/>
          Add Grocery
          </Link>
          <Link href={"/admin/view-grocery"} className='flex items-center gap-2 bg-white text-green-700 font-semibold
          px-4 py-2 rounded-full hover:bg-green-100 transition-all'>
            <Boxes className='w-5 h-5'/>
          View Grocery
          </Link>
        ...
        <div className='relative' ref={profileDropDown}>
          ...
          overflow-hidden shadow-md hover:scale-105 transition-transform relative' onClick={() => setOpen(prev => !prev)}>
            {user.image ? <Image src={user.image} alt="user" fill className="object-cover rounded-full" />
              : <User />}
          </div>
          <AnimatePresence>
            {open && <motion.div
              ...
              className='absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-xl border border-gray-200
          p-3 z-999'>
            ...
            text-gray-700 font-medium' onClick={() => setOpen(prev => !prev)}>
              ...
              <button className='flex items-center gap-2 w-full text-left py-3 px-3 hover:bg-red-50
