...
function DeliveryChat({ orderId, deliveryBoyId }: props) {
    ...
    const getSuggestion = async () => {
        ...
        const result = await axios.post("/api/chat/aiSuggestions", { message: lastMessage?.text, role: "delivery_boy" })
        ...
        <div className='bg-white rounded-3xl shadow-lg border p-4 h-107.5 flex flex-col'>
            <div className='flex justify-between items-center mb-3'>
                ...
                <motion.button
                    ...
                    onClick={getSuggestion}><Sparkle size={18} />
                    {loading?<Loader className='w-5 h-5 animate-spin'/>:"AI Suggest"}</motion.button>
            </div>
                ...
                <AnimatePresence>
                    {messages.map((msg, index) => (
                        <motion.div key={index} initial={{ opacity: 0, y: 15 }}
                            ...
                            <div className={`px-4 py-2 max-w-[75%] rounded-2xl shadow ${msg.senderId.toString() == deliveryBoyId ?
