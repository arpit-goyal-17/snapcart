...
function MyOrder() {
  ...
  useEffect(()=>{
    const getMyOrders=async ()=>{
      try {
        const result=await axios.get("/api/user/myOrders");
  ...
  useEffect(():any=>{
    ...
    socket.on("order-assigned",(data)=>{
      const {orderId, assignedDeliveryBoy} = data
      setOrders((prev)=>prev?.map((o)=>(o._id==orderId?{...o,assignedDeliveryBoy}:o)))
    })
    return ()=>socket.off("order-assigned")
  },[])
  if(loading)
    return <div className='flex items-center justify-center min-h-[50vh] text-gray-600'>Loading Your Order...</div>
  return (
