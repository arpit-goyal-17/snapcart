...
function TrackOrder({ params }: { params: { orderId: string } }) {
  ...
  const getSuggestion=async ()=>{
    ...
    const result=await axios.post("/api/chat/aiSuggestions",{message:lastMessage?.text,role:"user"})
  ...
  useEffect(() => {
    const getOrder = async () => {
      try {
        const result = await axios.get(`/api/user/getOrder/${orderId}`)
  ...
  useEffect((): any => {
    ...
    socket.on("update-deliveryBoy-location", ({ userId, location }) => {
      if (userId.toString() == order?.assignedDeliveryBoy?._id?.toString())
        setdeliveryLocation((prev) => ({ latitude: location.coordinates[1], longitude: location.coordinates[0] }))
    })
  ...
  return (
    <div className='w-full min-h-screen bg-linear-to-b from-green-50 to-white'>
      <div className='max-w-2xl mx-auto pb-24'>
        <div className='sticky top-0 bg-white/80 blackdrop-blur-xl p-4 border-b shadow flex gap-3 items-center z-999'>
          ...
          <div className='bg-white rounded-3xl shadow-lg border p-4 h-107.5 flex flex-col'>
            <div className='flex justify-between items-center mb-3'>
              ...
              <motion.button
                ...
                onClick={getSuggestion}><Sparkle size={18} />{loading?<Loader className='w-5 h-5 animate-spin'/>:"AI Suggest"}</motion.button>
            </div>
            ...
            <div className='flex-1 overflow-y-auto p-2 space-y-3' ref={chatBoxRef}>
              <AnimatePresence>
                {messages.map((msg, index) => (
                  <motion.div key={index} initial={{ opacity: 0, y: 15 }}
                    animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}
                    ...
                    <div className={`px-4 py-2 max-w-[75%] rounded-2xl shadow ${msg.senderId.toString() == userData?._id ?
                      ...
                      <p>{msg.text}</p>
                      <p className='text-2 opacity-70 mt-1 text-right'>{msg.time}</p>
                    </div>
                  </motion.div>
                ))}
              </AnimatePresence>
            ...
            <div className='flex gap-2 mt-3 border-t pt-3'>
              <input type="text" placeholder='Type a Message...' className='flex-1 bg-gray-100 px-4 py-2 rounded-xl outline-none focus:ring-2 focus:ring-green-500' onChange={(e) => setNewMessage(e.target.value)} value={newMessage} />
              <button className='bg-green-600 hover:bg-green-700 p-3 rounded-xl' onClick={sendMsg}><Send size={18} /></button>
            </div>
          </div>
        </div>
      </div>
