...
function Login() {
  ...
  return (
    <div className='flex flex-col items-center justify-center min-h-screen px-6 py-10 bg-white relative'>
      ...
      animate={{y:0,opacity:1}}
      transition={{duration:0.6}}
      className='text-gray-500 flex items-center gap-1'>Login to Snapcart <Leaf className='w-5 t-5 text-green-600'/>
      </motion.p>
      <br></br>
      <motion.form
       ...
       className='flex flex-col gap-5 w-full max-w-sm'>
        ...
        {(()=>{
          ...
          inline-flex items-center justify-center gap-2 ${
            ...
            {loading?<Loader2 className='w-5 h-5 animate-spin'/>:"Login In"}
          </button>
        })()}
      ...
      animate={{opacity:1}}
      transition={{duration:0.6}} className='max-w-sm w-full flex items-center justify-center gap-3 border
      ...
      >Want to create an account?<LogIn className='w-4 h-4'/> <span className='text-green-600 cursor-pointer'> Sign up</span></p>
    </div>
  )
}
